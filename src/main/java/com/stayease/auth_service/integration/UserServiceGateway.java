...
public class UserServiceGateway {
    ...
    public void createUserFallback(UserProfileRequest request,Exception ex){
        ...
        throw new RuntimeException("User Service is temporarily unavailable. Please try again later.",ex);
    }

    @Retry(name = "user-service",fallbackMethod = "verifyUserFallback")
    @CircuitBreaker(name = "user-service")
    ...
    @Retry(name="user-service",fallbackMethod="deleteUserFallback")
    @CircuitBreaker(name="user-service")
    ...
    public void deleteUserFallback(Long userId,Exception ex){
        ...
        throw new RuntimeException("Unable to rollback User Service.",ex);
    }
}